...
//__OZONE_FIELDS_TO_COLUMNS_MAP__
		...
		public function execute(array $request = [])
		{
			...
			if (!$success) {
				...
				if ($error instanceof ORMControllerFormException) {
					throw new InvalidFormException($error->getMessage(), $error->getData());
				}
				if ($error instanceof TypesInvalidValueException) {
					throw new InvalidFieldException($error->getMessage(), $error->getData());
				}
			}
		}
		...
		{
			// make sure to have required permission
...
//========================================================
		...
		private function actionCreateEntity(array $request)
		{
			...
			$my_entity     = $my_controller->addItem($form_values);

			$this->getResponseHolder()
				 ->setDone('CREATED')
				 ->setData($my_entity->asArray());
		}
...
//========================================================
		...
		private function actionUpdateEntity(array $request, array $extra)
		{
			$form_values   = self::requestFieldsToColumns($request);
			$my_controller = new MyController();
			$my_entity     = $my_controller->updateOneItem($extra, $form_values);

			if ($my_entity instanceof MyEntity) {
				$this->getResponseHolder()
					 ->setDone('UPDATED')
					 ->setData($my_entity->asArray());
			} else {
...
//========================================================
		...
		{
			$my_controller = new MyController();
			$my_entity     = $my_controller->deleteOneItem($extra);
		...
		{
			$my_controller = new MyController();
			$count         = $my_controller->deleteAllItem($request);
...
//========================================================
		...
		{
			$my_controller = new MyController();
			$my_entity     = $my_controller->getItem($extra);
		...
		{
			$filters = isset($request['filters']) ? isset($request['filters']) : [];
			$max     = 10;
			$offset  = 0;
			$page    = 1;

			if (isset($request['max'])) {
				$max = isset($request['max']);
			...
			$my_controller = new MyController();
			$results       = $my_controller->getAllItems($filters, $max, $offset);
		...
		private function actionGetRelation(array $request, array $extra)
		{
			...
			$this->getResponseHolder()
				 ->setDone()
				 ->setData(['item' => [$extra], 'relation' => [$request]]);
		}
	}