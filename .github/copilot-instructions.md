...
| `src/DBAL/Db.php`                                     | RDBMS base, `ns()`, `loadSchema()` (lazy with `setLazySchema()`: each table built on first use; `getTableFullName()` builds none), column ref resolution         |
...
| `src/DBAL/MetaMerger.php`                             | Metadata merge without dot-path parsing when no key needs it (`@internal`)                                                                                       |
...
| `src/ORM/ORMTableQuery.php`                           | Query/filter builder, `find()`, `select()`; `__call()` serves the `where*()` filters from the generated `FILTER_METHODS`                                         |
...
| `src/CRUD/CRUDEventProducer.php`                      | `listen()`, `onBefore*`, `onAfter*` subscriptions; creating one does not build its table                                                                         |
